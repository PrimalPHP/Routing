 ...
 */
	...
	public function testParseURL($url, $filter_paired, $process_in, $process_out, $find_route_in, $find_route_out, $check_routes, $final_name, $final_path, $arguments, $original_url) {
		...
		$obj = $this->getMockBuilder('\Primal\Routing\Router')
			->setMethods(array(
				'_validateRoutesPath',
				'_processSegments',
				'_findRoute',
 ...
 */
	...
	public function dataForProcessSegments() {
		return array(
			array(
				false, //pair all
				//input
			...
			array(
				false, //pair all
				//input
				array('alpha', 'beta=13', 'charley=november', 'delta=false', 'echo'),
				//indexed
				array('alpha', 'beta', 'charley', 'delta', 'echo'),
				//named
				array('beta'=>'13', 'charley'=>'november', 'delta'=>'false'),
				//map
				array('alpha'=>0, 'beta'=>1, 'charley'=>2, 'delta'=>3, 'echo'=>4),
			),
			array(
	...
	/**
	 * @dataProvider dataForProcessSegments
	 */
	public function testProcessSegments($pair_all, $input, $indexed, $named, $map) {

		$obj = new \Primal\Routing\Router();

		$result = $this->invokeMethod($obj, '_processSegments', $input, $pair_all);

		$this->assertEquals($indexed, $result[0], 'Indexed arguments');
		$this->assertEquals($named, $result[1], 'Named arguments');
		$this->assertEquals($map, $result[2], 'Map');
